...
public class MainActivity extends Activity {
	...
	private boolean validateCheckAmount() {
		...
		if (!checkAmount.getText().toString()
				.matches("^([0-9]*\\.[0-9][0-9])$")) {
			return false;
		}
		try {
	...
	private void calculateTip() {
		if (!validateCheckAmount()) {
			checkAmount
					.setError("Check amount must be in the format \"22.22\"");
			return;
		}
		...
		String tipAmountStr = "$"
				+ getFormattedDollarAmount(
						Float.valueOf(getCheckAmount() * desiredTip)).trim();
		System.err.println(tipAmountStr);
		tipAmount.setText(tipAmountStr);
	}

	private void setupRadioListeners() {
		tipOptions.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			@Override
